...
public:
    ...
    /*!
     * \name Реализация интерфейса модели.
     *
     ...
     * \note Q_DECL_OVERRIDE — это макроконстанта (\#define), которая заменяется
    ...
    //! Редактирует заметку \a note на позиции \a idx.
    void updateNoteAt(const Note &note, SizeType idx);
